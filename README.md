...
《舍友心晴：大学生宿舍压力预警与沟通演练助手》是一款面向大学生宿舍关系的网页版 AI 心理支持工具。项目聚焦宿舍关系压力、沟通回避和冲突积累等常见场景，当前完整流程为“事件记录 -> 事件档案 -> 宿舍总压力分析 -> AI 沟通模拟 -> 沟通复盘报告”，帮助用户在真实沟通前进行低风险表达练习。

当前仓库已完成前后端核心 Demo、V2 运行增强和 V3 持久化/复盘产品化迭代：已实现 `GET /health`、`POST /api/analyze`、`POST /api/events`、`GET /api/events`、`DELETE /api/events/{id}`、`GET /api/events/analysis`、`POST /api/events/insight`、`POST /api/simulate`、`POST /api/simulate/stream`、`POST /api/review`、`GET /api/reviews` 和 `GET /api/reviews/{review_id}`。前端已包含首页、事件记录、宿舍总压力分析、事件档案、AI 沟通模拟和沟通复盘页面；AI 接口通过 LangChain 调用 DeepSeek 官方 OpenAI 兼容 API，并使用结构化响应供前端展示。本地开发已补充 Vite `/api` 代理、FastAPI 本地 CORS、SQLite 事件档案、LangGraph SQLite 会话记忆、真实流式模拟接入、复盘历史保存和复盘报告导出。
...
| 目标核心流程 | 宿舍事件记录 -> 事件档案 -> 宿舍总压力分析 -> AI 多角色沟通模拟 -> 沟通复盘报告；当前已实现完整前端页面、真实后端接口和本地联调配置 |
...
| 宿舍总压力分析 | V3 已实现 | 基于事件档案内全部记录，按事件日期衰减、近 30 天累计和事件类型多样性生成总压力值、主要来源、趋势提示和趋势图 |
| 多情绪记录 | V2 已实现 | 事件记录支持 `emotions` 多选和 `primary_emotion` 主情绪，兼容旧 `emotion` 字段 |
| AI 多角色沟通模拟 | V3 已实现 | 后端通过 LangChain/DeepSeek 生成可配置舍友回复，支持自定义舍友画像、事件档案上下文、SQLite 会话记忆、流式前端展示和多轮模拟 |
| 沟通复盘报告 | V3 已实现 | 后端通过 LangChain/DeepSeek 分析模拟对话，输出表达优点、潜在问题、表现评分、多条改写建议、沟通计划，并保存复盘历史，支持 Markdown/图片导出和再练一次 |
...
5. 用户进入 AI 沟通模拟，选择场景、配置舍友画像，并输入沟通话术。
6. AI 舍友按画像和 SQLite 会话记忆多轮回应。
7. 用户完成练习后，系统生成沟通复盘报告、表现评分、沟通计划和多条优化话术；用户可查看历史复盘、导出报告或用推荐话术再练一次。
...
| 压力分析模块 | V3 已实现 | 展示基于事件档案的宿舍总压力、风险等级、主要来源、趋势图、AI 心晴见解和建议 |
| AI 沟通模拟模块 | V3 已实现 | 支持场景选择/自定义、舍友画像配置、事件档案上下文、流式多轮回复、对话缓存和会话失效一键重开 |
| 复盘报告模块 | V3 已实现 | 展示表达总结、表现评分、原话 vs 推荐话术、沟通计划、复盘历史、对话记录弹窗、导出和再练一次 |

### 后端模块

| 模块 | 当前状态 | 说明 |
| --- | --- | --- |
| API 接口层 | V3 已实现 | 已提供健康检查、单事件分析、事件档案、总压力分析、AI 心晴见解、沟通模拟、流式模拟、复盘和复盘历史接口；前端已配置本地 Vite 代理，后端已支持本地 CORS |
...
| 复盘历史模块 | V3 已实现 | 使用 SQLite 保存每次真实复盘报告，并提供历史列表和详情接口 |
...
| `/api/events/analysis` | GET | 基于全部事件档案返回宿舍总压力分析 |
| `/api/events/insight` | POST | 基于事件档案和总压力分析生成 AI 心晴见解 |
...
以下内容为 AI 已实现能力：`/api/simulate`、`/api/simulate/stream`、`/api/review` 和 `/api/events/insight` 通过 LangChain 调用 DeepSeek 官方 API，并返回便于前端展示的结构化数据。V3 已补充事件档案上下文、动态舍友画像、LangGraph SQLite 会话记忆、多轮流式模拟、复盘多条改写建议、沟通计划和复盘历史。
...
V3 已实现事件档案、宿舍总压力分析、多情绪记录、自定义舍友、多轮流式模拟、事件档案 AI 心晴见解、趋势图、复盘评分、复盘历史、报告导出和再练一次。详细说明见 [V2/V3 功能说明](./docs/v2-features.md)。
...
| 网页 Demo | 可演示事件记录、事件档案、宿舍总压力分析、AI 沟通模拟、沟通复盘报告；真实 AI 结果需要前端运行环境与 AI Key 配合验收 | 朱春雯 + 曹乐 |
