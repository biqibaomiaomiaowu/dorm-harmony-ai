...
以下内容为 AI 已实现能力：`/api/simulate`、`/api/simulate/stream`、`/api/review` 和 `/api/events/insight` 通过 LangChain 调用 DeepSeek 官方 API，并返回便于前端展示的结构化数据。V4 已将前端入口升级为 AI 沟通演练，包含固定分步场景训练和自定义演练；复盘请求支持增强版 `source_meta`，可在报告和历史中区分“场景训练”与“自定义演练”，并携带场景训练的舍友摘要、回复链范围和难度压力画像。
...
- 高难度边界：高级/挑战训练可模拟防御、回避、反问、推诿、冷处理、转移焦点和多人站队，但禁止辱骂、威胁、羞辱、歧视、操控或人格评价。
- 可操作性：建议必须具体、温和、可执行。
- 结构化响应的 `safety_note` 必须包含非诊断、非医学判断、非人格评价、不代表真实舍友想法，以及辅导员、心理老师或现实支持提示。

固定场景训练难度目录：初级为 1 位温和舍友；中级为 2 位舍友解释或轻微反驳；高级为 4 位舍友固执反驳、责任转移、冷处理或表面答应不承诺；挑战为 5 位舍友多人反问、站队、推诿、冷处理和责任转移交织。
