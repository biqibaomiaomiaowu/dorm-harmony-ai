...
当前仓库已完成前后端核心 Demo 和 V2 运行增强：已实现 `GET /health`、`POST /api/analyze`、`POST /api/events`、`GET /api/events`、`DELETE /api/events/{id}`、`GET /api/events/analysis`、`POST /api/events/insight`、`POST /api/simulate`、`POST /api/simulate/stream` 和 `POST /api/review`。前端已包含首页、事件记录、宿舍总压力分析、事件档案、AI 沟通模拟和沟通复盘页面；AI 接口通过 LangChain 调用 DeepSeek 官方 OpenAI 兼容 API，并使用结构化响应供前端展示。本地开发已补充 Vite `/api` 代理、FastAPI 本地 CORS、事件档案 JSON 存储、短期会话记忆和复盘字段兼容。
...
| 目标技术路线 | Vue + Python FastAPI + LangChain + 本地 JSON；当前已落地 FastAPI + 规则评分模型 + LangChain/DeepSeek 后端 AI 接口 + 事件档案 JSON 存储 |
...
| 事件档案 | V2 已实现 | 事件记录会写入后端本地 JSON，可在 `/archive` 查看、删除，并作为总压力分析和 AI 心晴见解的数据来源 |
| 压力晴雨表 | 第一阶段已实现后端规则分析 | 根据事件信息生成 0-100 的压力值、风险等级、主要矛盾来源和系统建议 |
| 宿舍总压力分析 | V2 已实现 | 基于事件档案内全部记录，按事件日期衰减、近 30 天累计和事件类型多样性生成总压力值、主要来源和趋势提示 |
| 多情绪记录 | V2 已实现 | 事件记录支持 `emotions` 多选和 `primary_emotion` 主情绪，兼容旧 `emotion` 字段 |
| AI 多角色沟通模拟 | V2 已实现 | 后端通过 LangChain/DeepSeek 生成可配置舍友回复，支持自定义舍友画像、事件档案上下文、短期会话记忆和多轮模拟 |
| 沟通复盘报告 | V2 已实现 | 后端通过 LangChain/DeepSeek 分析模拟对话，输出表达优点、潜在问题、表现评分、多条改写建议和后续行动建议 |
...
6. AI 舍友按画像和短期会话记忆多轮回应。
7. 用户完成练习后，系统生成沟通复盘报告、表现评分和多条优化话术。

## 技术方案

项目目标采用前后端分离架构。当前后端已落地 FastAPI 接口层、压力评分模块、事件档案 JSON 存储、安全提示逻辑、LangChain Prompt、DeepSeek 官方 OpenAI 兼容结构化调用和短期会话记忆；前端已落地完整页面路由和本地 `/api` 代理。

```text
用户浏览器
  -> Vue 前端页面
  -> Python FastAPI 后端
  -> 压力评分模块 / 事件档案 JSON 存储 / LangChain AI 模块
  -> DeepSeek 官方 API
```

当前运行范围为 Vue 3 + FastAPI + 规则评分模型 + JSON 事件档案 + LangChain/DeepSeek AI 服务。事件档案是演示级本地 JSON 存储，不包含账号体系、云端同步或多用户隔离。

### 前端目标模块

| 模块 | 当前状态 | 说明 |
| --- | --- | --- |
| 首页模块 | 已实现 | 展示作品名称、核心定位、流程入口和 V2 功能入口 |
| 事件记录模块 | 已实现 | 收集事件日期、类型、频率、严重程度、多情绪、沟通状态、冲突状态和描述 |
| 事件档案模块 | V2 已实现 | 展示已保存事件、单条压力快照、删除入口，并可跳转总压力分析 |
| 压力分析模块 | V2 已实现 | 展示基于事件档案的宿舍总压力、风险等级、主要来源、AI 心晴见解和建议 |
| AI 沟通模拟模块 | V2 已实现 | 支持场景选择/自定义、舍友画像配置、事件档案上下文、多轮回复和对话缓存 |
| 复盘报告模块 | V2 已实现 | 展示表达总结、表现评分、改写建议、后续建议和安全提示 |

### 后端模块

| 模块 | 当前状态 | 说明 |
| --- | --- | --- |
| API 接口层 | V2 已实现 | 已提供健康检查、单事件分析、事件档案、总压力分析、AI 心晴见解、沟通模拟、流式模拟和复盘接口；前端已配置本地 Vite 代理，后端已支持本地 CORS |
| 压力评分模块 | 第一阶段已实现 | 根据规则模型计算压力值和风险等级 |
| 安全边界模块 | 第一阶段已实现基础提示 | 对压力分析输出加入非诊断性提示和求助建议 |
| 事件档案存储模块 | V2 已实现 | 使用本地 JSON 文件保存演示事件，默认路径为 `backend/.runtime/events.json`，可通过 `DORM_HARMONY_EVENT_STORE_PATH` 覆盖 |
| LangChain 模块 | V2 已实现 | 管理提示词模板、动态舍友画像、事件档案摘要、短期会话记忆和 AI 调用流程 |
| AI 服务模块 | V2 已实现 | 调用 DeepSeek `deepseek-v4-flash` 生成多角色回复、复盘建议和事件档案心晴见解，缺少本地 API Key 时返回 `503` |
...
| `/api/events` | GET | 获取当前本地 JSON 事件档案 |
...
| `/api/review` | POST | 接收 `conversation_id` 或对话记录，返回沟通复盘报告 |
...
以下内容为 AI 已实现能力：`/api/simulate`、`/api/simulate/stream`、`/api/review` 和 `/api/events/insight` 通过 LangChain 调用 DeepSeek 官方 API，并返回便于前端展示的结构化数据。V2 已补充事件档案上下文、动态舍友画像、短期会话记忆、多轮模拟和复盘多条改写建议。
...
V2 已实现事件档案、宿舍总压力分析、多情绪记录、自定义舍友、多轮模拟、事件档案 AI 心晴见解和复盘评分。详细说明见 [V2 功能说明](./docs/v2-features.md)。

当前限制：

- 事件档案使用后端本地 JSON 文件保存，默认路径为 `backend/.runtime/events.json`；适合本地 Demo，不提供账号体系、云端同步、多用户隔离或数据库迁移能力。
- AI 会话记忆使用后端单进程内存保存，`conversation_id` 只在当前后端进程内有效；后端重启、换进程或使用旧 `conversation_id` 时，模拟和复盘可能返回“未找到对应的模拟对话”。
- 浏览器刷新后，前端会从 `localStorage` 恢复部分页面缓存和 `conversation_id`，但 conversation memory 只保存在后端单进程内，不能靠刷新持久化；后端重启、换进程或旧 id 会导致 memory 丢失，需要重新演练。
...
| V2 运行增强 | 追加迭代 | 实现事件档案、总压力分析、多情绪、自定义舍友、多轮模拟和复盘评分 | 事件档案 JSON 存储、`/archive` 页面、总压力分析页、AI 心晴见解、动态舍友画像、短期会话记忆、复盘表现评分和多条改写建议 |
