...
def analyze_archive_pressure(
    ...
    period_events = filter_events_by_period(
        all_events,
        analysis_date,
        normalized_period_days,
    )
    active_30d_count = _active_30d_count(all_events, analysis_date)
    if not period_events:
        return ArchiveAnalysisResponse(
            ...
            emotion_keywords=[],
            trend_message=(
                f"事件档案共记录 {len(all_events)} 条事件，"
                f"但近 {normalized_period_days} 天当前周期内暂无事件记录。"
            ),
            suggestion="当前周期内暂无事件记录，建议继续记录新的宿舍相处事件后再查看周期压力。",
            recommend_simulation=False,
            disclaimer=SAFETY_DISCLAIMER,
    ...
    active_period_count = len(period_events)
    accumulation_bonus = min(15, round(5 * log(1 + max(active_period_count - 1, 0))))
    ...
    source_insights = build_source_insights(period_events, source_breakdown, analysis_date)
    response = ArchiveAnalysisResponse(
        ...
        trend_message=(
            f"事件档案共记录 {len(all_events)} 条事件，"
            f"其中近 {normalized_period_days} 天 {active_period_count} 条。"
...
def _active_30d_count(events: list[EventRecord], analysis_date: date) -> int:
    """保留旧版近 30 天活跃事件计数口径，供兼容字段展示。"""
    return sum(
        1
        for event in events
        if max((analysis_date - event.event_date).days, 0) <= 30
    )
